...
src=HERE/'walkthrough.src.md'
backup=HERE/'walkthrough-v5.src.md'
if not backup.exists():backup.write_text(src.read_text())
old=backup.read_text()
inspection=old.split('## Same inspect function, different settings {#inspection-settings}\n')[1].split('## Individual calls and exact evidence {#calls}\n')[0]
inspection=inspection.replace('This workbook has none.','The original six-tab Man Group file has none. The new Fee lab experiment deliberately creates one.')
calls=old.split('## Individual calls and exact evidence {#calls}\n')[1]
calls=calls.replace('Runtime: `@oai/artifact-tool` 2.8.59, workspace bundle 26.909.12148. Everything here is local; no changes were pushed to main.','Runtime: `@oai/artifact-tool` 2.8.59, workspace bundle 26.909.12148. These are recorded experiments, not live workbook controls.')
calls=calls.replace('](man-group-run/source.xlsx)','](man-group-run/source.xlsx?download)').replace('](man-group-run/man-group-demo.xlsx)','](man-group-run/man-group-demo.xlsx?download)')
prefix='''---
...
history:
  - v6 2026-09-15 — Workbook lifecycle, 26 additional operations and explicit verification provenance
...
The 26 recorded steps cover six families below. Each distinguishes the **operation’s own return** from the **additional observations requested by the agent**. “Undefined” means no returned report; it does not mean that nothing happened.
...
'''
prefix=prefix.replace('26 additional operations','27 additional operations').replace('The 26 recorded steps','The 27 recorded steps')
prefix=prefix.replace('| **Verify saved or native features when relevant.**','| **Check that formal Table ranges do not overlap.** | Inspect existing ranges before creating another Table. | The API accepts our deliberately overlapping Table in memory. The agent must enforce this documented constraint. [Experiment](#op-overlap-table). |\n| **Verify saved or native features when relevant.**')
src.write_text(prefix+inspection+'\n### Original step-by-step sequence — exact calls and added checks {#calls}\n\n'+calls)

root=Path('/Users/ruggerogargiulo/.codex/plugins/cache/openai-primary-runtime/spreadsheets/26.909.12148/skills/spreadsheets')
selections=[('SKILL.md','Verification',[(327,333),(364,368),(385,387)]),('SKILL.md','Formula and model design',[(129,140),(175,189)]),('workflows/edit_workflows.md','Preserving an existing workbook',[(1,24)]),('artifact_tool_docs/API_QUICK_START.md','Automatic calculation and bounded discovery',[(33,45),(81,87),(274,282)]),('features/charts.md','Charts and verification',[(11,16),(37,43)])]
lines=['# Installed spreadsheet guidance: saved excerpts','','Captured 15 September 2026 from workspace bundle 26.909.12148. These are instructions supplied to the agent, not executable guarantees. Line numbers refer to the original installed files. Full source-file SHA-256 values identify the edition.','']
manifest=[]
for file,title,ranges in selections:
    p=root/file;alllines=p.read_text().splitlines();digest=hashlib.sha256(p.read_bytes()).hexdigest()
    lines+=['## '+title,'','Source: `'+str(p)+'`','', 'SHA-256: `'+digest+'`','']
    for start,end in ranges:
        lines+=['Lines '+str(start)+'–'+str(end)+':','','```text',*alllines[start-1:end],'```','']
    manifest.append({'source':str(p),'sha256':digest,'lineRanges':ranges})
(HERE/'guidance.md').write_text('\n'.join(lines))
(HERE/'evidence/guidance-provenance.json').write_text(json.dumps(manifest,indent=2))
